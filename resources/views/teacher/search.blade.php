...
@section("content")
    <div class="row">
        <p class="h2">{{__("Resultat des recherches pour : ")}} <span>{{ $word }}</span></p>
        <hr>
    </div>
    <div class="row">
        @if (count($projects) > 0)
                ...
                <div class="col">
                    <div class="card mb-5" style="width: 18rem;">
                        ...
                        <div class="card-footer">
                            <a href="{{route("teacher.groups.show", ['id' => $project['project']->id, 'classe_id' => $project['classe']])}}" class="btn btn-primary w-100">{{__("Voir le projet")}}</a>
                        </div>
                    </div>
                </div>
            @endforeach
